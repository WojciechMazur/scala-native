...
// non-public
    ...
    override def spliterator(): java.util.concurrent.ConcurrentHashMap.KeySpliterator[K,V] = {
      var t: Array[Node[K, V]] = null
      val m = map
      val n = m.sumCount
      val f =
  ...
  private[concurrent] final class ValuesView[K <: AnyRef, V <: AnyRef] private[concurrent] (
    ...
    override final def add(e: V): Nothing = throw new UnsupportedOperationException

    override final def addAll(c: Collection[_ <: V]): Nothing =
      throw new UnsupportedOperationException
