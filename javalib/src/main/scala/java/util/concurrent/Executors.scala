 ...
 *  {@link ScheduledExecutorService}, {@link ThreadFactory}, and {@link
 *  Callable} classes defined in this package. This class supports the following
 *  kinds of methods:
 *
 *  <ul> <li>Methods that create and return an {@link ExecutorService} set up
 *  with commonly useful configuration settings. <li>Methods that create and
 *  return a {@link ScheduledExecutorService} set up with commonly useful
 *  configuration settings. <li>Methods that create and return a "wrapped"
 *  ExecutorService, that disables reconfiguration by making
 *  implementation-specific methods inaccessible. <li>Methods that create and
 *  return a {@link ThreadFactory} that sets newly created threads to a known
 *  state. <li>Methods that create and return a {@link Callable} out of other
...
object Executors {
  ...
  /** Creates a work-stealing thread pool using the number of {@linkplain
   *  Runtime#availableProcessors() available processors} as its target
   *  parallelism level.
   *
   *  @return
   *    the newly created thread pool
   ...
   *  thread when needed. Unlike the otherwise equivalent {@code
   *  newFixedThreadPool(1, threadFactory)} the returned executor is guaranteed
   ...
   *  the cache. Thus, a pool that remains idle for long enough will not consume
   *  any resources. Note that pools with similar properties but different
   *  details (for example, timeout parameters) may be created using {@link
   ...
   *  {@code newScheduledThreadPool(1)} the returned executor is guaranteed not
   ...
   *  @return
   *    an {@code ExecutorService} instance
   *  @throws NullPointerException
   *    if executor null
   */
   ...
   *  ScheduledExecutorService} methods to the given executor, but not any other
   *  methods that might otherwise be accessible using casts. This provides a
   *  way to safely "freeze" configuration and disallow tuning of a given
   ...
   *  creates all new threads used by an Executor in the same {@link
   *  ThreadGroup}. If there is a {@link java.lang.SecurityManager}, it uses the
   *  group of {@link System#getSecurityManager}, else the group of the thread
   *  invoking this {@code defaultThreadFactory} method. Each new thread is
   *  created as a non-daemon thread with priority set to the smaller of {@code
   ...
   *  factory, and <em>M</em> is the sequence number of the thread created by
   *  this factory.
   *  @return
   *    a thread factory
   */
   ...
   *
   *  <p>Note that while tasks running within such threads will have the same
   *  access control and class loader settings as the current thread, they need
   *  not have the same {@link java.lang.ThreadLocal} or {@link
   *  java.lang.InheritableThreadLocal} values. If necessary, particular values
   *  of thread locals can be set or reset before any task runs in {@link
   *  ThreadPoolExecutor} subclasses using {@link
   ...
   *  method should normally be invoked within an {@link
   *  AccessController#doPrivileged AccessController.doPrivileged} action to
   ...
   *  an associated {@link AccessControlException}.
   *  @param callable
   *    the underlying task
   *  @param <T>
   ...
   *  that action; or if not possible, throw an associated {@link
   ...
   *  @throws AccessControlException
   *    if the current access control context does not have permission to both
   *    set and get context class loader
   */
