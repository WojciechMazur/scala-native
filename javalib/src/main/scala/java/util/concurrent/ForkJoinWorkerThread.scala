...
/** A thread managed by a {@link ForkJoinPool}, which executes {@link
 *  ForkJoinTask}s. This class is subclassable solely for the sake of adding
 *  functionality -- there are no overridable methods dealing with scheduling or
 *  execution. However, you can override initialization and termination methods
 *  surrounding the main task processing loop. If you do create such a subclass,
 *  you will also need to supply a custom {@link
 *  ForkJoinPool.ForkJoinWorkerThreadFactory} to {@linkplain
...
class ForkJoinWorkerThread private[concurrent] (
   ...
   *
   *  @param group
   *    if non-null, the thread group for this thread
   *  @param pool
   ...
   *  tasks. If you override this method, you must invoke {@code
   *  super.onStart()} at the beginning of the method. Initialization requires
   *  care: Most fields must have legal default values, to ensure that attempted
   *  accesses from other threads work correctly even before this thread starts
   *  processing tasks.
   */
  protected def onStart(): Unit = ()

  /** Performs cleanup associated with termination of this worker thread. If you
   *  override this method, you must invoke {@code super.onTermination} at the
   *  end of the overridden method.
   *
   *  @param exception
