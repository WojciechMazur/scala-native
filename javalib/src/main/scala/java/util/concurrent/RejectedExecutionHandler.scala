...
trait RejectedExecutionHandler {
   ...
   *  ThreadPoolExecutor#execute execute} cannot accept a task. This may occur
   *  when no more threads or queue slots are available because their bounds
   ...
   *  {@link RejectedExecutionException}, which will be propagated to the caller
   *  of {@code execute}.
   *
   *  @param r
