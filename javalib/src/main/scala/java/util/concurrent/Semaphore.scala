 ...
 *  blocking acquirer. However, no actual permit objects are used; the {@code
 ...
 *  that uses a semaphore to control access to a pool of items: <pre> {@code
 *  class Pool { private static final int MAX_AVAILABLE = 100; private final
 ...
 *
 *  protected synchronized Object getNextAvailableItem() { for (int i = 0; i <
 *  MAX_AVAILABLE; ++i) { if (!used[i]) { used[i] = true; return items[i]; } }
 *  return null; // not reached }
 *
 *  protected synchronized boolean markAsUnused(Object item) { for (int i = 0; i
 *  < MAX_AVAILABLE; ++i) { if (item == items[i]) { if (used[i]) { used[i] =
 *  false; return true; } else return false; } } return false; } }}</pre>
 *
 ...
 *  returned to the semaphore, allowing another thread to acquire that item.
 *  Note that no synchronization lock is held when {@link #acquire} is called as
 *  that would prevent an item from being returned to the pool. The semaphore
 *  encapsulates the synchronization needed to restrict access to the pool,
 *  separately from any synchronization needed to maintain the consistency of
 *  the pool itself.
 *
 *  <p>A semaphore initialized to one, and which is used such that it only has
 *  at most one permit available, can serve as a mutual exclusion lock. This is
 *  more commonly known as a <em>binary semaphore</em>, because it only has two
 *  states: one permit available, or zero permits available. When used in this
 *  way, the binary semaphore has the property (unlike many {@link
 *  java.util.concurrent.locks.Lock} implementations), that the &quot;lock&quot;
 *  can be released by a thread other than the owner (as semaphores have no
 *  notion of ownership). This can be useful in some specialized contexts, such
 *  as deadlock recovery.
 *
 *  <p>The constructor for this class optionally accepts a <em>fairness</em>
 ...
 *  of a thread that has been waiting - logically the new thread places itself
 *  at the head of the queue of waiting threads. When fairness is set true, the
 ...
 *  {@code acquire} before another, but reach the ordering point after the
 *  other, and similarly upon return from the method. Also note that the untimed
 ...
 *
 *  <p>This class also provides convenience methods to {@link #acquire(int)
 *  acquire} and {@link #release(int) release} multiple permits at a time. These
 *  methods are generally more efficient and effective than loops. However, they
 *  do not establish any preference order. For example, if thread A invokes
 *  {@code s.acquire(3}) and thread B invokes {@code s.acquire(2)}, and two
 *  permits become available, then there is no guarantee that thread B will
 *  obtain them unless its acquire came first and Semaphore {@code s} is in fair
 *  mode.
 *
 *  <p>Memory consistency effects: Actions in a thread prior to calling a
 *  "release" method such as {@code release()} <a
...
class Semaphore private (sync: Semaphore.Sync) extends Serializable {
   ...
   *
   *  @param permits
   *    the initial number of permits available. This value may be negative, in
   ...
   *  happens: <ul> <li>Some other thread invokes the {@link #release} method
   *  for this semaphore and the current thread is next to be assigned a permit;
   *  or <li>Some other thread {@linkplain Thread#interrupt interrupts} the
   *  current thread. </ul>
   *
   ...
   *  waiting for a permit, </ul> then {@link InterruptedException} is thrown
   ...
   *  time of invocation.
   *
   *  <p>Acquires a permit, if one is available and returns immediately, with
   *  the value {@code true}, reducing the number of available permits by one.
   *
   *  <p>If no permit is available then this method will return immediately with
   *  the value {@code false}.
   *
   *  <p>Even when this semaphore has been set to use a fair ordering policy, a
   *  call to {@code tryAcquire()} <em>will</em> immediately acquire a permit if
   *  one is available, whether or not other threads are currently waiting. This
   *  &quot;barging&quot; behavior can be useful in certain circumstances, even
   *  though it breaks fairness. If you want to honor the fairness setting, then
   *  use {@link #tryAcquire(long, TimeUnit) tryAcquire(0, TimeUnit.SECONDS)}
   ...
   *  for this semaphore and the current thread is next to be assigned a permit;
   *  or <li>Some other thread {@linkplain Thread#interrupt interrupts} the
   *  current thread; or <li>The specified waiting time elapses. </ul>
   *
   *  <p>If a permit is acquired then the value {@code true} is returned.
   *
   *  <p>If the current thread: <ul> <li>has its interrupted status set on entry
   *  to this method; or <li>is {@linkplain Thread#interrupt interrupted} while
   *  waiting to acquire a permit, </ul> then {@link InterruptedException} is
   ...
   *  @return
   *    {@code true} if a permit was acquired and {@code false} if the waiting
   ...
   *
   *  <p>Releases a permit, increasing the number of available permits by one.
   ...
   *  returns immediately, reducing the number of available permits by the given
   *  amount. This method has the same effect as the loop {@code for (int i = 0;
   *  i < permits; ++i) acquire();} except that it atomically acquires the
   *  permits all at once:
   *
   *  <p>If insufficient permits are available then the current thread becomes
   *  disabled for thread scheduling purposes and lies dormant until one of two
   *  things happens: <ul> <li>Some other thread invokes one of the {@link
   *  #release() release} methods for this semaphore and the current thread is
   *  next to be assigned permits and the number of available permits satisfies
   *  this request; or <li>Some other thread {@linkplain Thread#interrupt
   *  interrupts} the current thread. </ul>
   *
   *  <p>If the current thread: <ul> <li>has its interrupted status set on entry
   *  to this method; or <li>is {@linkplain Thread#interrupt interrupted} while
   *  waiting for a permit, </ul> then {@link InterruptedException} is thrown
   *  and the current thread's interrupted status is cleared. Any permits that
   *  were to be assigned to this thread are instead assigned to other threads
   *  trying to acquire permits, as if permits had been made available by a call
   *  to {@link #release()}.
   *
   *  @param permits
   ...
   *  returns immediately, reducing the number of available permits by the given
   *  amount. This method has the same effect as the loop {@code for (int i = 0;
   *  i < permits; ++i) acquireUninterruptibly();} except that it atomically
   *  acquires the permits all at once:
   *
   *  <p>If insufficient permits are available then the current thread becomes
   ...
   *  in the queue is not affected. When the thread does return from this method
   ...
   *  &quot;barging&quot; behavior can be useful in certain circumstances, even
   *  though it breaks fairness. If you want to honor the fairness setting, then
   *  use {@link #tryAcquire(int, long, TimeUnit) tryAcquire(permits, 0,
   ...
   *
   *  <p>Acquires the given number of permits, if they are available and returns
   ...
   *  three things happens: <ul> <li>Some other thread invokes one of the {@link
   *  #release() release} methods for this semaphore and the current thread is
   *  next to be assigned permits and the number of available permits satisfies
   *  this request; or <li>Some other thread {@linkplain Thread#interrupt
   *  interrupts} the current thread; or <li>The specified waiting time elapses.
   *  </ul>
   *
   ...
   *  <p>If the current thread: <ul> <li>has its interrupted status set on entry
   *  to this method; or <li>is {@linkplain Thread#interrupt interrupted} while
   *  waiting to acquire the permits, </ul> then {@link InterruptedException} is
   *  thrown and the current thread's interrupted status is cleared. Any permits
   *  that were to be assigned to this thread, are instead assigned to other
   *  threads trying to acquire permits, as if the permits had been made
   ...
   *    the time unit of the {@code timeout} argument
   *  @return
   *    {@code true} if all permits were acquired and {@code false} if the
   ...
   *  satisfied, then those permits are assigned in turn to other threads trying
   *  to acquire permits.
   *
   *  <p>There is no requirement that a thread that releases a permit must have
   *  acquired that permit by calling {@link Semaphore#acquire acquire}. Correct
   ...
   *  guarantee that any other thread will ever acquire. This method is designed
