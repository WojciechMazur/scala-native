 ...
 *
 *  <p> The simplest implementation of this interface is just: <pre> {@code
 *  class SimpleThreadFactory implements ThreadFactory { public Thread
 *  newThread(Runnable r) { return new Thread(r); } }}</pre>
 *
...
trait ThreadFactory {
   ...
   *  @return
   *    constructed thread, or {@code null} if the request to create a thread is
   *    rejected
   */
