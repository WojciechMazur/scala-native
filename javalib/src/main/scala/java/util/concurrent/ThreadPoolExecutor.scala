 ...
 *  improved performance when executing large numbers of asynchronous tasks, due
 *  to reduced per-task invocation overhead, and they provide a means of
 *  bounding and managing the resources, including threads, consumed when
 *  executing a collection of tasks. Each {@code ThreadPoolExecutor} also
 ...
 *  urged to use the more convenient {@link Executors} factory methods {@link
 ...
 *  {@link #getCorePoolSize}) and maximumPoolSize (see {@link
 *  #getMaximumPoolSize}).
 *
 *  When a new task is submitted in method {@link #execute(Runnable)}, if fewer
 *  than corePoolSize threads are running, a new thread is created to handle the
 *  request, even if other worker threads are idle. Else if fewer than
 *  maximumPoolSize threads are running, a new thread will be created to handle
 *  the request only if the queue is full. By setting corePoolSize and
 ...
 *  upon construction, but they may also be changed dynamically using {@link
 ...
 *  threads to all be in the same {@link ThreadGroup} and with the same {@code
 *  NORM_PRIORITY} priority and non-daemon status. By supplying a different
 *  ThreadFactory, you can alter the thread's name, thread group, priority,
 *  daemon status, etc. If a {@code ThreadFactory} fails to create a thread when
 *  asked by returning null from {@code newThread}, the executor will continue,
 *  but might not be able to execute any tasks. Threads should possess the
 *  "modifyThread" {@code RuntimePermission}. If worker threads or other threads
 *  using the pool do not possess this permission, service may be degraded:
 *  configuration changes may not take effect in a timely manner, and a shutdown
 *  pool may remain in a state in which termination is possible but not
 *  completed.</dd>
 *
 ...
 *  terminating prior to shut down. By default, the keep-alive policy applies
 *  only when there are more than corePoolSize threads, but method {@link
 *  #allowCoreThreadTimeOut(boolean)} can be used to apply this time-out policy
 *  to core threads as well, so long as the keepAliveTime value is non-zero.
 *  </dd>
 *
 *  <dt>Queuing</dt>
 *
 ...
 *  each others execution; for example, in a web page server. While this style
 *  of queuing can be useful in smoothing out transient bursts of requests, it
 ...
 *  and maximum pool sizes may be traded off for each other: Using large queues
 *  and small pools minimizes CPU usage, OS resources, and context-switching
 *  overhead, but can lead to artificially low throughput. If tasks frequently
 ...
 *  invokes the {@link RejectedExecutionHandler#rejectedExecution(Runnable,
 *  ThreadPoolExecutor)} method of its {@link RejectedExecutionHandler}. Four
 *  predefined handler policies are provided:
 *
 *  <ol>
 *
 *  <li>In the default {@link AbortPolicy}, the handler throws a runtime {@link
 *  RejectedExecutionException} upon rejection.
 *
 *  <li>In {@link CallerRunsPolicy}, the thread that invokes {@code execute}
 ...
 *
 *  <li>In {@link DiscardPolicy}, a task that cannot be executed is simply
 *  dropped. This policy is designed only for those rare cases in which task
 *  completion is never relied upon.
 *
 *  <li>In {@link DiscardOldestPolicy}, if the executor is not shut down, the
 *  task at the head of the work queue is dropped, and then execution is retried
 *  (which can fail again, causing this to be repeated.) This policy is rarely
 *  acceptable. In nearly all cases, you should also cancel the task to cause an
 *  exception in any component waiting for its completion, and/or log the
 *  failure, as illustrated in {@link DiscardOldestPolicy} documentation.
 *
 *  </ol>
 *
 ...
 *  #beforeExecute(Thread, Runnable)} and {@link #afterExecute(Runnable,
 *  Throwable)} methods that are called before and after execution of each task.
 *  These can be used to manipulate the execution environment; for example,
 *  reinitializing ThreadLocals, gathering statistics, or adding log entries.
 *  Additionally, method {@link #terminated} can be overridden to perform any
 *  special processing that needs to be done once the Executor has fully
 *  terminated.
 *
 *  <p>If hook, callback, or BlockingQueue methods throw exceptions, internal
 ...
 *  remaining threads may be reclaimed (garbage collected) without being
 *  explicitly shutdown. You can configure a pool to allow all unused threads to
 *  eventually die by setting appropriate keep-alive times, using a lower bound
 *  of zero core threads and/or setting {@link
 *  #allowCoreThreadTimeOut(boolean)}. </dd>
 *
 *  </dl>
 *
 *  <p><b>Extension example</b>. Most extensions of this class override one or
 *  more of the protected hook methods. For example, here is a subclass that
 *  adds a simple pause/resume feature:
 *
 *  <pre> {@code class PausableThreadPoolExecutor extends ThreadPoolExecutor {
 *  private boolean isPaused; private ReentrantLock pauseLock = new
 *  ReentrantLock(); private Condition unpaused = pauseLock.newCondition();
 *
 *  public PausableThreadPoolExecutor(...) { super(...); }
 *
 *  protected void beforeExecute(Thread t, Runnable r) { super.beforeExecute(t,
 *  r); pauseLock.lock(); try { while (isPaused) unpaused.await(); } catch
 *  (InterruptedException ie) { t.interrupt(); } finally { pauseLock.unlock(); }
 *  }
 *
 ...
 *  unpaused.signalAll(); } finally { pauseLock.unlock(); } } }}</pre>
 *
 *  @since 1.5
 *  @author
 *    Doug Lea
...
// Packing and unpacking ctl
   ...
   *
   *  This is the default handler for {@link ThreadPoolExecutor} and {@link
   ...
   *  which case the task is discarded. This policy is rarely useful in cases
   *  where other threads may be waiting for tasks to terminate, or failures
   *  must be recorded. Instead consider using a handler of the form: <pre>
   *  {@code new RejectedExecutionHandler() { public void
   *  rejectedExecution(Runnable r, ThreadPoolExecutor e) { Future<?> dropped =
   *  e.getQueue().poll(); if (dropped != null) dropped.cancel(false); // also
   *  consider logging the failure e.execute(r); // retry }}}</pre>
   */
  class DiscardOldestPolicy() extends RejectedExecutionHandler {
     ...
     *  execute, if one is immediately available, and then retries execution of
     *  task r, unless the executor is shut down, in which case task r is
     *  instead discarded.
     *
     *  @param r
     *    the runnable task requested to be executed
...
class ThreadPoolExecutor(
     ...
     *  do not require that workQueue.poll() returning null necessarily means
     *  that workQueue.isEmpty(), so rely solely on isEmpty to see if the queue
     *  is empty (which we must do for example when deciding whether to
     *  transition from SHUTDOWN to TIDYING). This accommodates special-purpose
     ...
     *  threads may result in new tasks being rejected or existing ones
     *  remaining stuck in the queue.
     *
   ...
   *  to use a lock. Among the reasons is that this serializes
   ...
   *  the associated statistics bookkeeping of largestPoolSize etc. We also hold
   *  mainLock on shutdown and shutdownNow, for the sake of ensuring workers set
   *  is stable while separately checking permission to interrupt and actually
   *  interrupting.
   */
   ...
   *  instead interrupting a task being run. We implement a simple non-reentrant
   *  mutual exclusion lock rather than use ReentrantLock because we do not want
   *  worker tasks to be able to reacquire the lock when they invoke pool
   *  control methods like setCorePoolSize. Additionally, to suppress interrupts
   *  until the thread actually starts running tasks, we initialize lock state
   *  to a negative value, and clear it upon start (in runWorker).
   */
   ...
   *  might make termination possible -- reducing worker count or removing tasks
   *  from the queue during shutdown. The method is non-private to allow access
   *  from Scheduled
   */
   ...
   *    tryTerminate when termination is otherwise enabled but there are still
   *    other workers. In this case, at most one waiting worker is interrupted
   *    to propagate shutdown signals in case all threads are currently waiting.
   *    Interrupting any arbitrary thread ensures that newly arriving workers
   *    since shutdown began will also eventually exit. To guarantee eventual
   *    termination, it suffices to always interrupt only one idle worker, but
   *    shutdown() interrupts all idle workers so that redundant workers exit
   ...
   *  Package-protected for use by Scheduled
   */
   ...
   *  the given bound (either core or maximum). If so, the worker count is
   ...
   *  the pool is stopped or eligible to shut down. It also returns false if the
   *  thread factory fails to create a thread when asked. If the thread creation
   *  fails, either due to the thread factory returning null, or due to an
   *  exception (typically OutOfMemoryError in Thread.start()), we roll back
   *  cleanly.
   *
   *  @param firstTask
   *    the task the new thread should run first (or null if none). Workers are
   *    created with an initial first task (in method execute()) to bypass
   *    queuing when there are fewer than corePoolSize threads (in which case we
   *    always start one), or when the queue is full (in which case we must
   *    bypass queue). Initially idle threads are usually created via
   *    prestartCoreThread or to replace other dying workers.
   *
   *  @param core
   *    if true use corePoolSize as bound, else maximumPoolSize. (A boolean
   *    indicator is used here rather than a value to ensure reads of fresh
   ...
   *    - rechecks for termination, in case the existence of this worker was
   *      holding up termination
   */
   ...
   *       setMaximumPoolSize). 2. The pool is stopped. 3. The pool is shutdown
   *       and the queue is empty. 4. This worker timed out waiting for a task,
   *       and timed-out workers are subject to termination (that is, {@code
   *       allowCoreThreadTimeOut || workerCount > corePoolSize}) both before
   *       and after the timed wait, and if the queue is non-empty, this worker
   *       is not the last thread in the pool.
   *
   *  @return
   *    task, or null if the worker must exit, in which case workerCount is
   *    decremented
   */
   ...
   *       get the first one. Otherwise, as long as pool is running, we get
   *       tasks from getTask. If it returns null then the worker exits due to
   *       changed pool state or configuration parameters. Other exits result
   *       from exception throws in external code, in which case
   ...
   *
   *  3. Each task run is preceded by a call to beforeExecute, which might throw
   *  an exception, in which case we cause thread to die (breaking loop with
   *  completedAbruptly true) without processing the task.
   *
   *  4. Assuming beforeExecute completes normally, we run the task, gathering
   *  any of its thrown exceptions to send to afterExecute. We separately handle
   *  RuntimeException, Error (both of which the specs guarantee that we trap)
   *  and arbitrary Throwables. Because we cannot rethrow Throwables within
   ...
   *  exception, which will also cause thread to die. According to JLS Sec
   ...
   *  parameters, the {@linkplain Executors#defaultThreadFactory default thread
   *  factory} and the {@linkplain AbortPolicy default rejected execution
   *  handler}.
   *
   *  <p>It may be more convenient to use one of the {@link Executors} factory
   ...
   *    time that excess idle threads will wait for new tasks before
   *    terminating.
   *  @param unit
   *    the time unit for the {@code keepAliveTime} argument
   *  @param workQueue
   *    the queue to use for holding tasks before they are executed. This queue
   *    will hold only the {@code Runnable} tasks submitted by the {@code
   *    execute} method.
   *  @throws IllegalArgumentException
   *    if one of the following holds:<br> {@code corePoolSize < 0}<br> {@code
   *    keepAliveTime < 0}<br> {@code maximumPoolSize <= 0}<br> {@code
   ...
   *  and the {@linkplain AbortPolicy default rejected execution handler}.
   *
   *  @param corePoolSize
   *    the number of threads to keep in the pool, even if they are idle, unless
   *    {@code allowCoreThreadTimeOut} is set
  ...
  /** Creates a new {@code ThreadPoolExecutor} with the given initial parameters
   *  and the {@linkplain Executors#defaultThreadFactory default thread
   *  factory}.
   *
   *  @param corePoolSize
   *    the number of threads to keep in the pool, even if they are idle, unless
   ...
   *  task is handled by the current {@link RejectedExecutionHandler}.
   *
   *  @param command
   *    the task to execute
   *  @throws RejectedExecutionException
   ...
   *  terminated. This method may be useful for debugging. A return of {@code
   *  true} reported a sufficient period after shutdown may indicate that
   *  submitted tasks have ignored or suppressed interruption, causing this
   ...
   *    the new core size
   *  @throws IllegalArgumentException
   *    if {@code corePoolSize < 0} or {@code corePoolSize} is greater than the
   ...
   *  This method will return {@code false} if all core threads have already
   *  been started.
   *
   *  @return
  ...
  /** Returns true if this pool allows core threads to time out and terminate if
   *  no tasks arrive within the keepAlive time, being replaced if needed when
   ...
   *
   *  @return
   *    {@code true} if core threads are allowed to time out, else {@code false}
   *
   *  @since 1.6
   */
   ...
   *  @throws IllegalArgumentException
   *    if value is {@code true} and the current keep-alive time is not greater
   *    than zero
   *
   *  @since 1.6
   */
  ...
  /** Sets the thread keep-alive time, which is the amount of time that threads
   *  may remain idle before being terminated. Threads that wait this amount of
   *  time without processing a task will be terminated if there are more than
   *  the core number of threads currently in the pool, or if this pool
   *  {@linkplain #allowsCoreThreadTimeOut() allows core thread timeout}. This
   *  overrides any value set in the constructor.
   *
   *  @param time
   *    the time to wait. A time value of zero will cause excess threads to
   *    terminate immediately after executing tasks.
   *  @param unit
   *    the time unit of the {@code time} argument
   *  @throws IllegalArgumentException
   *    if {@code time} less than zero or if {@code time} is zero and {@code
   ...
   *  threads may remain idle before being terminated. Threads that wait this
   *  amount of time without processing a task will be terminated if there are
   *  more than the core number of threads currently in the pool, or if this
   *  pool {@linkplain #allowsCoreThreadTimeOut() allows core thread timeout}.
   *
   *  @param unit
   ...
   *
   *  <p>This method may be useful as one part of a cancellation scheme. It may
   ...
   *  Future} status. However, in such cases, method {@link #purge} may be used
   *  to remove those Futures that have been cancelled.
   *
   *  @param task
   *    the task to remove
   *  @return
   *    {@code true} if the task was removed
   */
   ...
   *  never executed, but may accumulate in work queues until worker threads can
   ...
   *  This method is invoked by thread {@code t} that will execute task {@code
   *  r}, and may be used to re-initialize ThreadLocals, or to perform logging.
   *
   ...
   *
   *  <p><b>Note:</b> When actions are enclosed in tasks (such as {@link
   *  FutureTask}) either explicitly or via methods such as {@code submit},
   *  these task objects catch and maintain computational exceptions, and so
   *  they do not cause abrupt termination, and the internal exceptions are
   *  <em>not</em> passed to this method. If you would like to trap both kinds
   *  of failures in this method, you can further probe for such cases, as in
   *  this sample subclass that prints either the direct cause or the underlying
   *  exception if a task has been aborted:
   *
   *  <pre> {@code class ExtendedExecutor extends ThreadPoolExecutor { // ...
   *  protected void afterExecute(Runnable r, Throwable t) {
   *  super.afterExecute(r, t); if (t == null && r instanceof Future<?> &&
   *  ((Future<?>)r).isDone()) { try { Object result = ((Future<?>) r).get(); }
   *  catch (CancellationException ce) { t = ce; } catch (ExecutionException ee)
   *  { t = ee.getCause(); } catch (InterruptedException ie) { // ignore/reset
   *  Thread.currentThread().interrupt(); } } if (t != null)
   *  System.out.println(t); } }}</pre>
   *
   *  @param r
   *    the runnable that has completed
   *  @param t
   *    the exception that caused termination, or null if execution completed
   *    normally
   */
