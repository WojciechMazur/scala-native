 ...
 *  {@code AbstractQueuedLongSynchronizer} method otherwise creates such a
 ...
 *  "fast-path" checks, possibly prechecking {@link #hasContended} and/or {@link
...
object AbstractQueuedLongSynchronizer { // Node status bits, also used as argument and return values
  ...
  abstract private[locks] class Node {
    private[locks] val prev =
      new AtomicReference[Node]() // initially attached via casTail
    private[locks] val next =
      new AtomicReference[Node]() // visibly nonnull when signallable
    private[locks] val waiter =
      new AtomicReference[Thread]() // visibly nonnull when enqueued
    private[locks] val status =
      new AtomicInteger(0) // written by owner, atomic bit ops by others
  ...
  final private[locks] class ConditionNode
    ...
    private[locks] var nextWaiter: ConditionNode =
      null // link to next waiting node
...
abstract class AbstractQueuedLongSynchronizer protected ()
   ...
   */
  protected val state = new AtomicLong(0L)
   ...
   *    current state value
   */
  final protected def getState(): Long = state.get()
  ...
  private def cleanQueue(): Unit = {
    ...
    while (!break) { // restart point
      ...
      while (true) {
        ...
        else if (q.status.get() < 0) { // canceled
          val casNode =
  ...
  private def cancelAcquire(
    ...
    if (node != null) {
      node.waiter.set(null)
      node.status.set(AbstractQueuedLongSynchronizer.CANCELLED)
      if (node.prev.get() != null) cleanQueue()
    }

    if (!interrupted) 0
    else {
      if (interruptible) AbstractQueuedLongSynchronizer.CANCELLED
      else {
  ...
  final def acquireInterruptibly(arg: Long): Unit = {
    if (Thread.interrupted() || (!tryAcquire(arg) && acquire(
          null,
          arg,
          false,
          true,
  ...
  final def release(arg: Long): Boolean = {
    if (tryRelease(arg)) {
      AbstractQueuedLongSynchronizer.signalNext(head.get())
      true
    } else false
  }
  ...
  class ConditionObject() extends Condition with Serializable {
    ...
    private def doSignal(
      ...
      @tailrec
      def loop(first: AbstractQueuedLongSynchronizer.ConditionNode): Unit = {
        if (first == null) ()
        else {
          ...
          if ((first.getAndUnsetStatus(
                AbstractQueuedLongSynchronizer.COND
    ...
    private def enableWait(
      ...
      node.status.set(
        AbstractQueuedLongSynchronizer.CANCELLED
      )
    ...
    private def unlinkCancelledWaiters(
      ...
      if (node == null || node.nextWaiter != null || (node == lastWaiter)) {
        ...
        while (w != null) {
          ...
          if ((w.status.get() & AbstractQueuedLongSynchronizer.COND) == 0) {
            w.nextWaiter = null
    ...
    override final def await(): Unit = {
      ...
      breakable {
        while (!canReacquire(node)) {
          interrupted |= Thread.interrupted()
          if (interrupted) {
            cancelled = (node.getAndUnsetStatus(
              AbstractQueuedLongSynchronizer.COND
            ) & AbstractQueuedLongSynchronizer.COND) != 0
            if (cancelled) {
      ...
      breakable {
        while (!canReacquire(node)) {
          ...
          if (interruptedOrDeadline()) {
            cancelled =
              (node.getAndUnsetStatus(AbstractQueuedLongSynchronizer.COND) &
                AbstractQueuedLongSynchronizer.COND) != 0
            if (cancelled) break() else LockSupport.parkNanos(this, nanos)
          }
        }
      }
      ...
      breakable {
        while (!canReacquire(node)) {
          ...
          if (interrupted || System.currentTimeMillis() >= abstime) {
            ...
            if (cancelled) break() else LockSupport.parkUntil(this, abstime)
          }
        }
      }
    ...
    final private[locks] def isOwnedBy(sync: AbstractQueuedLongSynchronizer) = {
      sync == AbstractQueuedLongSynchronizer.this
    }
