 ...
 * {@code AbstractQueuedLongSynchronizer} does not implement any
 ...
 * using an {@code AbstractQueuedLongSynchronizer} for their
...
abstract class AbstractQueuedLongSynchronizer protected ()
  ...
  final private[locks] def enqueue(
      node: AbstractQueuedLongSynchronizer.Node): Unit = {
    @tailrec
    def tryEnqueue(): Unit = {
      val t = tail.get()
  ...
  final private[locks] def isEnqueued(
      node: AbstractQueuedLongSynchronizer.Node): Boolean = {
    @tailrec
   ...
   */
  final private[locks] def acquire(_node: AbstractQueuedLongSynchronizer.Node,
                                   arg: Long,
                                   shared: Boolean,
   ...
   */
  private def cancelAcquire(node: AbstractQueuedLongSynchronizer.Node,
                            interrupted: Boolean,
   ...
   * avoid <a href="AbstractQueuedLongSynchronizer.html#barging">barging</a>.
   ...
   */
  final def owns(
      condition: AbstractQueuedLongSynchronizer#ConditionObject): Boolean =
  ...
  final def getWaitQueueLength(
      condition: AbstractQueuedLongSynchronizer#ConditionObject): Int = {
    if (!owns(condition))
      throw new IllegalArgumentException("Not owner")
  ...
  class ConditionObject() extends Condition with Serializable {
     ...
     */
    private def doSignal(first: AbstractQueuedLongSynchronizer.ConditionNode,
                         all: Boolean): Unit = {
      @tailrec
      def loop(first: AbstractQueuedLongSynchronizer.ConditionNode): Unit = {
        if (first == null) ()
          ...
          if ((first.getAndUnsetStatus(AbstractQueuedLongSynchronizer.COND) & AbstractQueuedLongSynchronizer.COND) != 0) {
            enqueue(first)
            if (!all) return ()
          }
    ...
    private def enableWait(
        node: AbstractQueuedLongSynchronizer.ConditionNode): Long = {
      if (isHeldExclusively()) {
    ...
    private def canReacquire(
        node: AbstractQueuedLongSynchronizer.ConditionNode) = { // check links, not status to avoid enqueue race
    ...
    private def unlinkCancelledWaiters(
        node: AbstractQueuedLongSynchronizer.ConditionNode): Unit = {
    ...
    override final def await(): Unit = {
      ...
      breakable {
        while (!canReacquire(node)) {
          ...
          if (interrupted) {
            cancelled =
              (node.getAndUnsetStatus(AbstractQueuedLongSynchronizer.COND) & AbstractQueuedLongSynchronizer.COND) != 0
            if (cancelled) {
