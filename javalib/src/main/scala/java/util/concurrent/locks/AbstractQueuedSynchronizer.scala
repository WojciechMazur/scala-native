 ...
 *  synchronization interface. Instead it defines methods such as {@link
 *  #acquireInterruptibly} that can be invoked as appropriate by concrete locks
 ...
 *  may (but need not) succeed. This class does not &quotunderstand&quot these
 ...
 *  these modes, but both can come into play for example in a {@link
 *  ReadWriteLock}. Subclasses that support only exclusive or only shared modes
 *  need not define the methods supporting the unused mode.
 *
 *  <p>This class defines a nested {@link ConditionObject} class that can be
 *  used as a {@link Condition} implementation by subclasses supporting
 *  exclusive mode for which method {@link #isHeldExclusively()} reports whether
 *  synchronization is exclusively held with respect to the current thread,
 *  method {@link #release} invoked with the current {@link #getState()} value
 *  fully releases this object, and {@link #acquire}, given this saved state
 *  value, eventually restores this object to its previous acquired state. No
 ...
 *  state using {@link #getState()}, {@link #setState} and/or {@link
 *  #compareAndSetState}:
 *
 *  <ul> <li>{@link #tryAcquire} <li>{@link #tryRelease} <li>{@link
 *  #tryAcquireShared} <li>{@link #tryReleaseShared} <li>{@link
 *  #isHeldExclusively()} </ul>
 *
 *  Each of these methods by default throws {@link
 *  UnsupportedOperationException}. Implementations of these methods must be
 *  internally thread-safe, and should in general be short and not block.
 ...
 *
 *  <pre> <em>Acquire:</em> while (!tryAcquire(arg)) { <em>enqueue thread if it
 *  is not already queued</em> <em>possibly block current thread</em> }
 *
 *  <em>Release:</em> if (tryRelease(arg)) <em>unblock the first queued
 *  thread</em> </pre>
 *
 ...
 *  tryAcquire} to return {@code false} if {@link #hasQueuedPredecessors} (a
 *  method specifically designed to be used by fair synchronizers) returns
 ...
 *  succeed against incoming threads. Also, while acquires do not &quotspin&quot
 *  in the usual sense, they may perform multiple invocations of {@code
 ...
 *  possibly prechecking {@link #hasContended} and/or {@link #hasQueuedThreads}
 ...
 *  isLocked() { return sync.isLocked() } public boolean isHeldByCurrentThread()
 *  { return sync.isHeldExclusively() } public boolean hasQueuedThreads() {
 *  return sync.hasQueuedThreads() } public void lockInterruptibly() throws
 *  InterruptedException { sync.acquireInterruptibly(1) } public boolean
 *  tryLock(long timeout, TimeUnit unit) throws InterruptedException { return
 *  sync.tryAcquireNanos(1, unit.toNanos(timeout)) } }}</pre>
 *
 *  <p>Here is a latch class that is like a {@link
...
abstract class AbstractQueuedSynchronizer protected ()
  ...
  final protected def compareAndSetState(
      expected: Int,
      newState: Int
  ): Boolean =
  ...
  final private[locks] def acquire(
    ...
    def getPred() = {
      pred = if (node == null) null else node.prev
      pred
    }
    def isFirst() = {
      first = head == pred
    ...
    val continue = new scala.util.control.Breaks()
    while (true) {
      continue.breakable {
        if (!first && getPred() != null && !isFirst()) {
          if (pred.status < 0) {
  ...
  private def cleanQueue(): Unit = while (true) {
    ...
    breakable {
      ...
      while (true) {
        ...
        if (p == null) return () // ned of list

        val isIncosisient =
          if (s == null) tail != q
          else s.prev != q || s.status < 0
   ...
   *    otherwise uninterpreted and can represent anything you like.
   *  @return
   *    a negative value on failure zero if acquisition in shared mode succeeded
   *    but no subsequent shared-mode acquire can succeed and a positive value
   *    if acquisition in shared mode succeeded and subsequent shared-mode
   *    acquires might also succeed, in which case a subsequent waiting thread
   *    must check availability. (Support for three different return values
   *    enables this method to be used in contexts where acquires only sometimes
   ...
   *  a {@link ConditionObject} method.
   *
   *  <p>The default implementation throws {@link
   *  UnsupportedOperationException}. This method is invoked internally only
   *  within {@link ConditionObject} methods, so need not be defined if
   ...
   *
   *  @param arg
   *    the acquire argument. This value is conveyed to {@link #tryAcquire} but
   ...
   *  #tryAcquire}, returning on success. Otherwise the thread is queued,
   *  possibly repeatedly blocking and unblocking, invoking {@link #tryAcquire}
   ...
   *  thread is interrupted or the timeout elapses. This method can be used to
   *  implement method {@link Lock#tryLock(long, TimeUnit)}.
   *
   *  @param arg
   *    the acquire argument. This value is conveyed to {@link #tryAcquire} but
   *    is otherwise uninterpreted and can represent anything you like.
   ...
   *
   *  @param arg
   *    the release argument. This value is conveyed to {@link #tryRelease} but
   *    is otherwise uninterpreted and can represent anything you like.
   *  @return
   ...
   *
   *  @param arg
   *    the acquire argument. This value is conveyed to {@link
   ...
   *  checking interrupt status, then invoking at least once {@link
   *  #tryAcquireShared}, returning on success. Otherwise the thread is queued,
   ...
   *
   *  @param arg
   *    the release argument. This value is conveyed to {@link
   *    #tryReleaseShared} but is otherwise uninterpreted and can represent
   *    anything you like.
   *  @return
   ...
   *  its {@link #tryAcquireShared} method should return a negative value, if
   *  this method returns {@code true} (unless this is a reentrant acquire). For
   *  example, the {@code tryAcquire} method for a fair, reentrant, exclusive
   *  mode synchronizer might look like this:
   *
   *  <pre> {@code protected boolean tryAcquire(int arg) { if
   *  (isHeldExclusively()) { // A reentrant acquire increment hold count return
   *  true } else if (hasQueuedPredecessors()) { return false } else { // try to
   *  acquire normally } }}</pre>
   *
   *  @return
  ...
  class ConditionObject() extends Condition with Serializable {
    ...
    private def doSignal(
        _first: AbstractQueuedSynchronizer.ConditionNode,
        all: Boolean
    ): Unit = {
      var first = _first
      while (first != null) {
     ...
     *  returned by {@link #getState}. <li>Invoke {@link #release} with saved
     *  state as argument, throwing IllegalMonitorStateException if it fails.
     *  <li>Block until signalled. <li>Reacquire by invoking specialized version
     *  of {@link #acquire} with saved state as argument. </ol>
     */
     ...
     *  {@link #getState}. <li>Invoke {@link #release} with saved state as
     *  argument, throwing IllegalMonitorStateException if it fails. <li>Block
     *  until signalled or interrupted. <li>Reacquire by invoking specialized
     *  version of {@link #acquire} with saved state as argument. <li>If
     ...
     *  {@link #getState}. <li>Invoke {@link #release} with saved state as
     *  argument, throwing IllegalMonitorStateException if it fails. <li>Block
     *  until signalled, interrupted, or timed out. <li>Reacquire by invoking
     *  specialized version of {@link #acquire} with saved state as argument.
     ...
     *  specialized version of {@link #acquire} with saved state as argument.
     *  <li>If interrupted while blocked in step 4, throw InterruptedException.
     *  <li>If timed out while blocked in step 4, return false, else true. </ol>
     */
    @throws[InterruptedException]
     ...
     *  this Condition. Implements {@link
