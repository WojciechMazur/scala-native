 ...
 *  already owns the lock. This can be checked using methods {@link
 ...
 *  holding the lock. Also note that the untimed {@link #tryLock()()} method
 *  does not honor the fairness setting. It will succeed if the lock is
 ...
 *
 *  <p>In addition to implementing the {@link Lock} interface, this class
 *  defines a number of {@code public} and {@code protected} methods for
...
class ReentrantLock private (sync: ReentrantLock.Sync)
   ...
   *  <li>is {@linkplain Thread#interrupt interrupted} while acquiring the lock,
   *
   *  </ul> then {@link InterruptedException} is thrown and the current thread's
   *  interrupted status is cleared.
   *
   ...
   *  the current thread is not the holder of this lock then {@link
   ...
   *  instance.
   *
   *  <p>The returned {@link Condition} instance supports the same usages as do
   *  the {@link Object} monitor methods ({@link Object#wait() wait}, {@link
   *  Object#notify notify}, and {@link Object#notifyAll notifyAll}) when used
   *  with the built-in monitor lock.
   *
   *  <ul>
   *
   *  <li>If this lock is not held when any of the {@link Condition} {@linkplain
   ...
   *
   *  <p>Analogous to the {@link Thread#holdsLock(Object)} method for built-in
   ...
   *  <pre> {@code class X { ReentrantLock lock = new ReentrantLock(); // ...
   *
   *  public void m() { assert lock.isHeldByCurrentThread()(); // ... method
   *  body } }}</pre>
   *
