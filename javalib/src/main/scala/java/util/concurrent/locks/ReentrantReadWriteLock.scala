 ...
 *  {@link ReentrantLock}. <p>This class has the following properties:
 *
 ...
 *  are no waiting threads). (Note that the non-blocking {@link
 *  ReadLock#tryLock()} and {@link WriteLock#tryLock()} methods do not honor
 *  this fair setting and will immediately acquire the lock if it is possible,
 *  regardless of waiting threads.) </dl>
 *
 *  <li><b>Reentrancy</b>
 *
 *  <p>This lock allows both readers and writers to reacquire read or write
 *  locks in the style of a {@link ReentrantLock}. Non-reentrant readers are not
 ...
 *
 *  <li><b>{@link Condition} support</b> <p>The write lock provides a {@link
 *  Condition} implementation that behaves in the same way, with respect to the
 *  write lock, as the {@link Condition} implementation provided by {@link
 *  ReentrantLock#newCondition} does for {@link ReentrantLock}. This {@link
 *  Condition} can, of course, only be used with the write lock.
 *
 *  <p>The read lock does not support a {@link Condition} and {@code
 ...
 *
 *  <pre> {@code class CachedData { Object data; boolean cacheValid; final
 ...
 *  rwl.writeLock().lock(); try { // Recheck state because another thread might
 *  have // acquired write lock and changed state before we did. if
 *  (!cacheValid) { data = ... cacheValid = true; } // Downgrade by acquiring
 *  read lock before releasing write lock rwl.readLock().lock(); } finally {
 ...
 *
 *  <pre> {@code class RWDictionary { private final Map<String, Data> m = new
 *  TreeMap<>(); private final ReentrantReadWriteLock rwl = new
 *  ReentrantReadWriteLock(); private final Lock r = rwl.readLock(); private
 *  final Lock w = rwl.writeLock();
 *
 *  public Data get(String key) { r.lock(); try { return m.get(key); } finally {
 *  r.unlock(); } } public List<String> allKeys() { r.lock(); try { return new
 *  ArrayList<>(m.keySet()); } finally { r.unlock(); } } public Data put(String
 *  key, Data value) { w.lock(); try { return m.put(key, value); } finally {
 *  w.unlock(); } } public void clear() { w.lock(); try { m.clear(); } finally {
 *  w.unlock(); } } }}</pre>
 *
 *  <h2>Implementation Notes</h2>
 *
 *  <p>This lock supports a maximum of 65535 recursive write locks and 65535
 *  read locks. Attempts to exceed these limits result in {@link Error} throws
...
object ReentrantReadWriteLock {
  ...
  class ReadLock private (final private val sync: ReentrantReadWriteLock.Sync)
     ...
     *  this lock has been set to use a fair ordering policy, a call to {@code
     ...
     *  lock. This is in contrast to the {@link #tryLock()} method. If you want
     *  a timed {@code tryLock} that does permit barging on a fair lock then
     ...
     *  read lock,
     *
     *  </ul> then {@link InterruptedException} is thrown and the current
  ...
  class WriteLock private (final private val sync: ReentrantReadWriteLock.Sync)
     ...
     *  effect to {@link ReentrantReadWriteLock#isWriteLockedByCurrentThread}.
     *
     *  @return
     ...
     *  an unlock action. Identical in effect to {@link
...
class ReentrantReadWriteLock(val fair: Boolean)
   ...
   *
   *  @return
   *    {@code true} if the current thread holds the write lock and {@code
   *    false} otherwise
   */
   ...
   *  at any time, a {@code true} return does not guarantee that a future {@code
