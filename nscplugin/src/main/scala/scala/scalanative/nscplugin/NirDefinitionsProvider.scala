...
object NirDefinitionsProvider {
  trait Compat[Symbol, Name] {
    ...
    def TermName(name: String): Name

    def BoxesRunTimeModule: Symbol
    def ScalaRunTimeModule: Symbol
    def StringClass: Symbol
    def BoxedUnit_UNIT: Symbol
    def Array_clone: Symbol

    def symbolCompanion(sym: Symbol): Symbol
...
class NirDefinitionsProvider[Symbol, Name](val compat: NirDefinitionsProvider.Compat[Symbol, Name]) {
  ...
  lazy val EqEqMethodName = TermName("$eq$eq")
  lazy val NotEqMethodName = TermName("$bang$eq")
  ...
  lazy val RuntimeArrayModule: Map[Char, Symbol] =
    RuntimeArrayClass.map(mapValue(symbolCompanion(_)))
  ...
  ) ++ symbolAlternatives(getMember(ScalaRunTimeModule, TermName("hash")))
  ...
  lazy val Reflect_registerLoadableModuleClass =
    getMemberMethod(ReflectModule, TermName("registerLoadableModuleClass"))
  lazy val Reflect_registerInstantiatableClass =
    getMemberMethod(ReflectModule, TermName("registerInstantiatableClass"))
