...
class ThreadPoolExecutorTest extends JSR166Test {
  ...
  @Test def testGetQueue(): Unit = {
    ...
    usingWrappedPoolCleaner(p) { cleaner(_, done) } { p =>
      val threadStarted = new CountDownLatch(1)
      val tasks = new Array[FutureTask[Boolean]](5)
  ...
  //       def loop(): Unit =
  //         try done.await()
  //         catch { case ignored: InterruptedException => loop() }
  //       loop()
  //     }
  //     threadsStarted.countDown()
  //     foo()
  ...
  //   }
  //   assertEquals(
  //     JSR166Test.saturatedSize(p) - (if (shutdownNow)
  ...
  //  */
  // @throws[Exception]
  // @Test def testSubmitRunnable(): Unit = usingPoolCleaner(
  //   new ThreadPoolExecutor(
  //     2,
  //     2,
  //     LONG_DELAY_MS,
  ...
  //           false
  //         } catch { case ignore: RejectedExecutionException => true }
  //       }) ()
  //     }
  ...
  //   if (!JSR166Test.testImplementationDetails) return
