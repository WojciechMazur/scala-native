...
class ReentrantLockTest extends JSR166Test {
  ...
  @Test def testBlockers(): Unit = {
    ...
    val syncClassName =
      if (fair) "ReentrantLock$FairSync"
      else "ReentrantLock$NonfairSync"
    ...
    //   def foo() = {
    //     lazy val className = blockerClassName(thread)
    //     (thread.getState eq expectedAcquireState) &&
    //       className != null &&
    ...
    // lock.unlock()
    // val waitingForCondition: Runnable = () => {
    //   def foo() = {
    //     lazy val className = blockerClassName(thread)
    //     (thread.getState eq expectedAwaitState) &&
    //       className != null &&
