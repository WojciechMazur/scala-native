...
object MinWinBase {
  ...
  implicit class FileTimeStructOps(ref: Ptr[FileTimeStruct]) {
    ...
    def fileTime: FileTime = {
      HelperMethods.dwordPairToULargeInteger(high = highFileTime,
                                             low = lowFileTime)
    }
    def lowFileTime_=(v: DWord): Unit  = ref._1
    def highFileTime_=(v: DWord): Unit = ref._2
    def fileTime_=(v: FileTime): Unit = {
      HelperMethods.uLargeIntegerToDWordPair(v, high = ref.at1, low = ref.at2)
    }
  }
  ...
  implicit class SystemTimeOps(ref: Ptr[SystemTime]) {
